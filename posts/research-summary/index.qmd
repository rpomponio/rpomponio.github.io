...
*In this post, I attempt to review the beginnings of my research career and distill everything into three themes spanning topics in the biomedical sciences and applied statistics.*

# 2018 to 2023: Five years of research, Three themes

As we're nearing the end of the calendar year, I found it a good opportunity to present my research in a blog post and identify three different themes across the statistical and biomedical applications that I've worked on. Those themes are **computational**, **clinical**, and **missing data** methods. I've been fortunate to co-author a number of published papers over the past five years, and I've included links to relevant articles where they fit within the above themes. I'll cover each theme in more detail below.

## Theme 1: Computational Research

![Figure from Pomponio et al. (2020): Age trends for selected brain volumes using a statistically harmonized dataset with 18 studies spanning the age range 3–96.](trends.jpeg){width=70%}

### Computational Neuroimaging Research

In 2018, I began working at [CBICA](https://www.med.upenn.edu/cbica/), a computational neuroimaging lab at the University of Pennsylvania, led by Dr. Christos Davatzikos. At CBICA, I developed an early expertise in statistical image analysis, particularly in the challenges of normalizing imaging-derived features over multiple scanners and geographic sites. In the imaging literature, this is referred to as [harmonization](https://www.sciencedirect.com/science/article/pii/S1053811922006851#:~:text=Harmonization%20is%20typically%20performed%20to,downstream%20application%20performance%20during%20harmonization.), and there are a growing number of techniques available to implement harmonization techniques. My major contribution was to develop a tool that adapted the popular ComBat[^1] algorithm to lifespan datasets encompassing vast age ranges, with potentially complex age trends in brain volumetry. Today, that tool is available on [GitHub](https://github.com/rpomponio/neuroHarmonize) as a python package, and it is used by neuroimaging researchers to handle common harmonization tasks like removing site effects.

In addition to my work on harmonization, I assisted with image analysis in several other projects that resulted in peer-reviewed publications by the following first authors:
...
## Theme 3: Missing Data

The last theme of my research deals with missing data. Missing data are common yet complicated by enormous heterogeneity; missing data can arise at various stages within a study, and they present so many challenges that practitioners often ignore missing data altogether. In my work at Colorado, I encountered missing data in observational studies usually due to participant dropout or morbidity. When working with this kind of missing data, I typically used multiple imputation by chained equations[^2] to replace missing values with values that would be realistic given the remaining data. This technique works well under certain assumptions and is well-implemented in`R` by the [mice](https://github.com/amices/mice) package.

However, another kind of missing data can arise in studies investigating pre-post differences that fail to collect unique identifiers for each participant. For my master's thesis, I analyzed techniques to handle this particular case of statistical inference, which we refer to as 'partially matched' data. My work on this subject is currently under review, but the pre-print is below, co-authored by my thesis committee:
...
Looking back at the past five years, it's clear I've primarily worked on academic research projects with investigations into biomedical diseases and health interventions. Broadly, I've developed an expertise as a biostatistician and data scientist in medical research contexts.

If I were to speculate on the direction of the next five years, I would predict much of the same kinds of work, but with expansions into new contexts such as business and sports analytics. These areas excite me with an opportunity to integrate my undergraduate business experience with cutting edge data science skills. I've begun to work in these areas as an independent consultant through [Upwork](https://www.upwork.com/freelancers/~01bb85ee6f09078536), and I hope that I can continue to find clients who value my insight on diverse and wide-ranging projects.

Lastly, I look forward to beginning as a data scientist at [SygnaMap](https://www.sygnamap.com), a bio-tech startup based in San Antonio. At SygnaMap I will be helping to develop novel biomarkers using metabolomic profiles, which will draw on many of my past experiences in the themes above to deal with computational complexity, clinical utility, and challenges arising from missing data.
