...
Instead of innings pitched, the number of batters faced is a reasonable proxy of in-game time. Pitchers may throw different numbers of pitches, but you can generally expect a pitcher to throw about 3.8 pitches per batter faced in the long run. Additionally, pitchers are often replaced before facing a new batter, not mid-way through an at-bat.[^7] Lastly, the number of batters faced can easily be converted to times through the order; simply divide the number of batters by nine. Thus the number of batters faced is a sensible choice for the unit of time.

What does the number of batters faced look like for the 1218 playoff starts in the dataset? The histogram below shows the distribution of batters faced for all playoff starts.
