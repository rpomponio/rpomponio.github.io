...
primates.cc <- lapply(primates, function(x) x[complete.cases(milk[, c(3, 7, 8)])])
fit.draft <- quap(
  alist(
    K ~ dnorm(mu, sigma),
    mu <- alpha + betaM * M + betaN * N,
    alpha ~ dnorm(0, 0.2),
    betaM ~ dnorm(0, 0.5),
    betaN ~ dnorm(0, 0.5),
    sigma ~ dexp(1)
...
plot(
  precis(fit.mcateg, depth=2, pars="alpha"),
